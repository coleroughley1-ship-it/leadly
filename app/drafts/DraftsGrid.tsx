  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
              ...
              >
                <input
                  ...
                  checked={
                    ...
                    rows.filter((r) => r.status === "draft").every((r) => selectedIds[r.id])
                  }
                />
              </th>
                  ...
                  <td style={{ padding: 10, whiteSpace: "nowrap" }}>
                    {r.status === "committed" ? (
                      <span style={{ opacity: 0.8 }}>
                        committed{r.committed_at ? ` · ${new Date(r.committed_at).toLocaleString()}` : ""}
                      </span>
                    ) : (
      ...
      <div style={{ fontSize: 12, opacity: 0.7, lineHeight: 1.4 }}>
        Notes: Metrics clamp to 1–5. Score/action is computed by <code>lead_drafts_scored</code>. Saving is debounced.
      </div>
    </div>
  )
}
